setwd('C:/Users/khyat/Downloads/PA/PA_AirBnB/Dataset')
...
forestforairb = randomForest(price~.,data = train,mtry = 5,importance=TRUE)
...
varImpPlot(forestforairb)

table(varImpPlot(forestforairb))

predTestAll = predict(forestforairb, test)
RMSEAll = sqrt(mean((predTestAll-test$price)^2))

RMSEAll
#top 4 variables - bedrooms,accomodates,bathrooms,guests_included
...
forestforNine = randomForest(price~accommodates+bathrooms+bedrooms+host_is_superhost+number_of_reviews+availability_30+availability_90+neighbourhood_cleansed+room_type,
...
varImpPlot(forestforNine)

table(varImpPlot(forestforNine))





predTest = predict(forestforNine, test)

# Checking classification accuracy



RMSE = sqrt(mean((predTest-test$price)^2))
